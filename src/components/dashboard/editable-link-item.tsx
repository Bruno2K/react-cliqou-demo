...
export const EditableLinkItem: React.FC<EditableLinkItemProps> = ({ link, onEdit, onDelete, onToggleActive }) => {
  ...
  return (
    <Card ref={setNodeRef} style={style} className="mb-2 sm:mb-3 bg-card shadow-sm">
      <CardContent className="p-2 sm:p-3">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
          
          {/* Link Info Section (Drag, Icon, Title, URL) */}
          <div className="flex items-center gap-2 flex-grow min-w-0">
            <Button variant="ghost" size="icon" {...attributes} {...listeners} className="cursor-grab touch-none flex-shrink-0 h-8 w-8 sm:h-9 md:h-10 sm:w-9 md:w-10 p-1" aria-label="Drag to reorder">
          ...
          <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:gap-2 flex-shrink-0 mt-1 sm:mt-0 sm:pl-0">
            {/* On mobile, this inner div groups switch and its label horizontally */}
            ...
            {/* This div groups buttons and aligns them to the right on mobile (within its column), and start on sm+ */}
            <div className="flex gap-1.5 sm:gap-2 justify-end pl-10 sm:pl-0 sm:justify-start">
              ...
              <Button variant="destructive" size="icon" onClick={() => onDelete(link.id)} aria-label="Delete link" className="h-7 w-7 sm:h-8 sm:w-8 p-1">
